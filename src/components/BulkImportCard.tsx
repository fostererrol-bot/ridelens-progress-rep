import { Check, AlertTriangle, Loader2, X, Copy, ChevronDown, ChevronUp } from "lucide-react";
...
export function BulkImportCard({ item, onUpdateField, onRemove }: BulkImportCardProps) {
  ...
  const renderField = (section: string, field: string, label: string, type: "number" | "text" = "number") => (
    <div key={field}>
      ...
      <Input
        type={type}
        value={(section === "root" ? (item.extraction as any)[field] : (item.extraction as any)[section]?.[field]) ?? ""}
        onChange={(e) =>
    ...
    <div className="rounded-xl border border-border bg-card overflow-hidden card-glow">
      ...
      <div className="flex items-center gap-3 p-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          {item.status === "ready" && item.extraction.confidence.overall < 0.85 && (
            <div className="flex items-center gap-1 mt-1">
              <AlertTriangle className="w-3 h-3 text-warning" />
              <span className="text-[10px] text-warning">
                Low confidence ({(item.extraction.confidence.overall * 100).toFixed(0)}%) — review before saving
              </span>
            </div>
          )}
          {item.error && (
